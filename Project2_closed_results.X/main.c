...
int main(void){
    ...
    uint16_t aux;
    float r;
    float adc1, adc2, temp1, temp2, avg, avg_temp;
    int set_temp = 0;
    ...
    while (1)
    {
        ...
        {
            //printf("\33[2K");
            printf("\n\r%2.1f %2.0f %3.1f", avg, temp2, duty);
        }
    }
    return 0;
}
...
float average2(float sample) {
    #define BUFFER_SIZE2 50

    static double buffer[BUFFER_SIZE2] = {0};
    static int index = 0;
    static int count = 0;
    double sum = 0.0;
    ...
    // add the sample to the circular buffer
    buffer[index] = sample;
    index = (index + 1) % BUFFER_SIZE2;
    if (count < BUFFER_SIZE2) {
        count++;
    }

    // compute the sum of the last BUFFER_SIZE samples
    for (int i = 0; i < count; i++) {
        sum += buffer[(index + i) % BUFFER_SIZE2];
    }
